$(document).ready(function () {
    ...
    $('#formAdd-BookingVH').submit(function (e) {
        // if( $('#formAdd-M').form('is valid') == false) {
        //             return false ;
        //         }
        var data = $("#formAdd-BookingVH").serialize();
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (data) {
                // alert('success');
                $('#ModalformAdd-BookingVH').modal('hide');
    ...
    $(".show-link-BookingVH").click(function () {
        // alert('click');
        var id = $(this).attr("id");
        $('#eform2').load('DML-Booking/showdata-BookingVH.php?show_id=' + id);
        $('#ModalformShow-BookingVH').modal('show');
    });
});