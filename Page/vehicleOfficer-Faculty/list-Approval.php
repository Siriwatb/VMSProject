    ...
    <table class="ui celled table" cellspacing="0" width="100%" id="approval-Table">
        ...
        <?php
            //   require_once '../../ConfigDB.php';
              ...
              $stmt = $conn->prepare("SELECT `UsingID`,`reqDate`,p.`Name` as perName ,of.`Name` as offName,`Start Date`,`End Time`,`End Date`,`Start Time`,`NumPeople`,`Destination`,`Appointment`,`Reason` 
                                      ...
                                      JOIN faculty as f ON f.ID = of.Office 
                                      Where f.Name = :Office AND  `Approve Status` = 2 "); //Where `Approve Status` = 2
            ...
            <tr>
              ...
              <td><?php 
              if (IsNullOrEmptyString($row['perName'])==0) {
                echo $row['perName']; 
              }elseif(IsNullOrEmptyString($row['perName'])!==0){
                echo $row['offName']; 
              }
                  // echo $row['perName']; 
              ?>
