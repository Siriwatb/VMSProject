        ...
        module = {
          ...
          create: {
            calendar: function () {
              ...
              var year = display.getFullYear()+543;
              ...
              for (p = 0; p < pages; p++) {
                ...
                var firstMonthDayColumn = (new Date(year-543, month, 1).getDay() - settings.firstDayOfWeek % 7 + 7) % 7;
                if (!settings.constantHeight && isDay) {
                  var requiredCells = new Date(year-543, month + 1, 0).getDate() + firstMonthDayColumn;
                ...
                var prevDate = new Date(year-543 - yearChange, month - monthChange, prevNextDay - dayChange, hour);
                var nextDate = new Date(year-543 + yearChange, month + monthChange, prevNextDay + dayChange, hour);

                var prevLast = isYear ? new Date(Math.ceil(year-543 / 10) * 10 - 9, 0, 0) :
                  isMonth ? new Date(year-543, 0, 0) : isDay ? new Date(year-543, month, 0) : new Date(year-543, month, day, -1);
                var nextFirst = isYear ? new Date(Math.ceil(year-543 / 10) * 10 + 1, 0, 1) :
                  isMonth ? new Date(year-543 + 1, 0, 1) : isDay ? new Date(year-543, month + 1, 1) : new Date(year-543, month, day + 1);
                ...
                i = isYear ? Math.ceil(year-543 / 10) * 10 - 9 : isDay ? 1 - firstMonthDayColumn : 0;
                for (r = 0; r < rows; r++) {
            ...
            keydown: function (event) {
              ...
              if (module.popup('is visible')) {
                if (event.keyCode === 37 || event.keyCode === 38 || event.keyCode === 39 || event.keyCode === 40) {
                  ...
                  var year = focusDate.getFullYear()+ 543 + (mode === 'year' ? increment : 0);
          ...
          set: {
            ...
            focusDate: function (date, refreshCalendar, updateFocus, updateRange) {
              ...
              if (isDay && date && oldFocusDate) {
                var yearDelta = date.getFullYear() - oldFocusDate.getFullYear()-543;
